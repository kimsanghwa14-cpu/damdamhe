...
메인을 국내증시 / 미국증시 / 시장 폭·심리로 묶고 밝은 배경에 표시합니다. PC는 같은 높이의 3열 카드(중간 크기 2열), 모바일은 작은 미리보기가 있는 간결한 1열 목록입니다. 상단에는 마지막 갱신 시각과 카테고리 탭을 표시하고, 갱신 지연은 발생한 경우에만 표시합니다. 카드의 이름이나 미리보기를 누르면 전체 원본 캡처를 확대해서 볼 수 있습니다. 홈 디자인은 src/dashboard.css에서 관리하며 확대 모드와 스캐너는 기존 어두운 화면을 유지합니다. 갱신 실패 시 이전 정상 이미지를 유지하며 원본 링크를 제공합니다. 메인 소개글과 브라우저 제목의 ‘담담히’를 삭제했습니다.
